...
debug ? "self.postMessage({ 'cmd': 'debug', 'msg': [fus, becart, diff, max, bcout] });" : "",
  ...
  function upTabs()
  {
    ...
    DOM.newNodes([
      ...
      ['cat_span1', 'span', { 'class': 'BWMselect' + (U.getP('leg') === '' ? ' disabled' : '') }, ['non'], { 'click': [setL, ''] }, 'cat_th0'],
      ['cat_span2', 'span', {}, [', '], {}, 'cat_th0'],
      ['cat_span3', 'span', { 'class': 'BWMselect' + (U.getP('leg') === 'L' ? ' disabled' : '') }, ['oui'], { 'click': [setL, 'L'] }, 'cat_th0'],
      ['cat_tr1', 'tr', {}, [], {}, 'cat'],
      ['cat_td0', 'td', {}, [], {}, 'cat_tr1'],
      ['main_tr1', 'tr', {}, [], {}, 'main'],
      ...
      (U.getP('sim') > 0 ? ['sim_th3', 'th', { 'class': 'BWMselect' }, ['◄'], { 'click': [moveS, -1] }, 'sim_tr0'] : ['sim_th3a', 'th', {}, [], {}, 'sim_tr0']),
          ...
          else
          {
            DOM.newNodes([
              ...
              (j < s.s.length - 1 ? ['idx_td3' + j + '_6', 'td', { 'class': 'BWMselect' },
                ['▼'], { 'click': [moveSel, [j, j + 1]] }, 'idx_tr3' + j] : ['idx_td3' + j + '_6', 'td', { 'class': 'BWMselect' },
                  [], {}, 'idx_tr3' + j]),
              (j > 0 ? ['idx_td3' + j + '_7', 'td', { 'class': 'BWMselect' },
                ['▲'], { 'click': [moveSel, [j, j - 1]] }, 'idx_tr3' + j] : ['idx_td3' + j + '_7', 'td', { 'class': 'BWMselect' },
                  [], {}, 'idx_tr3' + j]),
      ...
      { // saisie par liste
        ...
        for (var k = 0; k < sel.length; k++)
        {
          if (sel[k][0].length > 0)
          {
            ...
            DOM.newNodes([
              ['get_tr0' + k, 'tr', { 'class': 'tblheader' }, [], {}, 'get'],
              ['get_th0' + k + '_0', 'th', { 'class': 'BWMselect ' + (!!U.getP(sel[k][2]) ? 'enabled' : 'disabled')}, ['[' + (!!U.getP(sel[k][2]) ? '-' : '+') + ']'], { 'click': [show, sel[k][2]] }, 'get_tr0' + k],
              ['get_th0' + k + '_1', 'th', { 'colspan': '4' }, [sel[k][1] + ' (' + sel[k][0].length + ')'], {}, 'get_tr0' + k]
            ], rootIU);
            if (!!U.getP(sel[k][2]))
            {
              DOM.newNodes([
                ...
                ['get_th1' + k + '_3', 'th', { 'class': 'BWMtitle' }, ['Suffixe'], { 'click': [selectTri, 3] }, 'get_tr1' + k],
                (U.getP('setZone') == -1 || (isGo && U.getP('setZone') < 0)) ? ['get_th1' + k + '_4', 'th', {}, [], {}, 'get_tr1' + k] : ['get_th1' + k + '_4', 'th', { 'class': 'BWMselect heal' }, ['►►'], { 'click': [selectAll, sel[k][0]] }, 'get_tr1' + k],
                ['get_span11' + k, 'span', { 'class': 'BWMtriSelect' }, [(U.getP('triOrder') === 1 ? '▲' : '▼')], {}, 'get_th1' + k + '_' + U.getP('triCol').toString()]
              ], rootIU);
              for (var i = 0; i < sel[k][0].length; i++)
              {
                ...
                DOM.newNodes([
                  ['get_tr2' + k + '_' + i, 'tr', { 'class': 'BWMTR2' + (i % 2 === 0 ? '' : ' BWMeven') }, [], {}, 'get']
                ], rootIU);
   ...
   ******************************************************/
  var page = G.page();
  if (debug) console.debug('BWMstart: page, U.id(), U.name()', page, U.id(), U.name());
  if (page == 'pMixitem')
  {
    if (!isNull(U.id()))
    {
